...
const Calendar = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.title && formData.startTime && formData.endTime && formData.weekday) {
      ...
      const createdEvent = await createEvent(token, newEvent); // creates new event in database
      const updatedEvents = await fetchEvents(token); // gets events from database
      setEvents(updatedEvents); // updates events client side with those in database
                ...
                {days.map((weekday) => {
                  ...
                  return (
                    <div
                      ...
                      onClick={() => clickCalendar(weekday, time)}
                    >
                      {event && (
                        <div className="event">
